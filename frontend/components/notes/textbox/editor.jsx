  ...
  render() {
    ...
    if (this.props.tag.length > 0) {
        tags = this.props.tag.map(tag => {
          return (
            <TagObject key={`${tag.id}`} tag={tag} deleteTagNote={this.props.deleteTagNote} note={this.props.note} fetchTag={this.props.fetchTag}/>              
          );
        });
      }
    return (
      <div className="text-body-div">
        ...
        <div className="edit-tag-footer">
          <form onSubmit={(e) => this.createNewTag(e)} className="show-edit-tags">
            <button disabled={!this.state.tagInput} id="make-new-tag-btn">
