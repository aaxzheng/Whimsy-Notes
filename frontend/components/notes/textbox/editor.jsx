...
class Editor extends React.Component {
  ...
  createNewTag(e) {
    const tag = {tag: this.tagName, user_id: this.props.user.id, note_id:this.props.note.id};
  ...
  render() {
    ...
    if (this.props.tag.length > 0) {
        tags = this.props.tag.map(tag => {
          return (
            <div key={tag.id} className="edit-tag-body">
      ...
      <div className="text-body-div">
        ...
        <div className="edit-tag-footer">
          <form onSubmit={(e) => this.createNewTag(e)} className="show-edit-tags">
            ...
            <input id="edit-tags" type="text" onChange={this.setTagName} spellCheck="false" className="tag-input"></input>
            </div>
          </form>
        </div>
      </div>
